...
trait Jump
{
    ...
    public function success($msg = '', string $url = null, $data = '', int $wait = 2, array $header = [])
    {
        if (is_null($url) && isset($_SERVER["HTTP_REFERER"])) {
            ...
            $url = (strpos($url, '://') || str_starts_with($url, '/')) ? $url : route($url);
        }
        ...
        if ('html' == strtolower($type)) {
            ...
            return response($handler::render(config("app.dispatch_success_tmpl"), $result), 200, $header);
        } else {
            return json($result);
        }
    }
    ...
    public function error($msg = '', string $url = null, $data = '', int $wait = 3, array $header = [])
    {
        ...
        if ('html' == strtolower($type)) {
            ...
            return response($handler::render(config("app.dispatch_error_tmpl"), $result), 200, $header);
        } else {
            return json($result);
        }
    }
