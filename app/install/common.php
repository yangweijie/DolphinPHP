...
function check_env(){
    ...
    if(function_exists('disk_free_space')) {
        $disk_size = floor(disk_free_space(base_path()) / (1024*1024));
...
function check_dirfile(){
    ...
    if(!defined('INSTALL_APP_PATH'))
        define('INSTALL_APP_PATH', public_path());
...
function write_config($config){
    if(is_array($config)){
        //读取配置内容
        $conf = file_get_contents(app_path().'/data/database.tpl');
        // 替换配置项
...
function create_tables($db, $prefix = ''){
    // 读取SQL文件
    $sql = file_get_contents(app_path() . '/data/dolphin.sql');
